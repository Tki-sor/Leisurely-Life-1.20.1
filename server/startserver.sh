...
if ! command -v "${LLR_JAVA:-java}" >/dev/null 2>&1; then
...
if [ ! -d libraries ]; then
    ...
    "${LLR_JAVA:-java}" -jar "$INSTALLER" -installServer
fi

if [ "${LLR_INSTALL_ONLY:-false}" = "true" ]; then
    echo "INSTALL_ONLY: complete"
    exit 0
fi

JAVA_VERSION=$("${LLR_JAVA:-java}" -fullversion 2>&1 | awk -F '"' '/version/ {print $2}' | cut -d'.' -f1)
...
while true
do
    "${LLR_JAVA:-java}" @user_jvm_args.txt @libraries/net/minecraftforge/forge/1.20.1-$FORGE_VERSION/unix_args.txt nogui

    if [ "${LLR_RESTART:-true}" = "false" ]; then
        exit 0
    fi
